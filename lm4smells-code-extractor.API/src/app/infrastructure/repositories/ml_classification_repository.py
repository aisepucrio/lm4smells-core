...
class MLClassificationRepository:
    def __init__(self) -> None:
        self.db_connection = DatabaseConnection()

    def save_all(self, classification: MLClassification) -> None:
        try:
            with self.db_connection.get_connection() as connection:
                cursor = connection.cursor()
                cursor.execute(
                    """
                    INSERT INTO ml_codes (
                        id, file_name, classification, model_used,
                        element_name, element_type, confidence_score, metrics
                ...
                connection.commit()
                print(f"[INFO] ML Classification saved successfully: {classification.id}")
        except Exception as e:
            print(f"[ERROR] Failed saving ML classification: {e}")
    ...
    def get_all(self) -> List[dict]:
        try:
            with self.db_connection.get_connection() as connection:
                ...
                cursor.execute(
                    ...
                    SELECT id, file_name, classification, model_used,
                           ...
                           metrics, created_at FROM ml_codes
            ...
            print(f"[ERROR] Failed retrieving ML classifications: {e}")
            return []
    ...
    def get_by_file_name(self, file_name: str) -> List[dict]:
        try:
            ...
            print(f"[ERROR] Failed retrieving ML classifications by file_name: {e}")
            return []
